...
from .util.dissim import matching_dissim,num_TZ_dissim
...
def _labels_cost(Xnum, Xcat, centroids, num_dissim, cat_dissim, gamma, membship=None):
    ...
    n_points = Xnum.shape[0] # numpy.array的shape属性返回矩阵的长和宽，shape[0]表示矩阵有几行
    Xnum = check_array(Xnum) # sklearn的check_array，确认Xnum是二维非空矩阵
    ...
    cost = 0.
    labels = np.empty(n_points, dtype=np.uint16)
    num_costs = num_dissim(centroids[0],Xnum)
    for i in range(n_points):
        tot_costs = num_costs[i] + gamma * cat_dissim(centroids[1], Xcat[i], X=Xcat, membship=membship)
        clust = np.argmin(tot_costs)
        labels[i] = clust
        cost += tot_costs[clust]
    return labels,cost
...
def _k_prototypes_iter(Xnum, Xcat, centroids, cl_attr_sum, cl_memb_sum, cl_attr_freq,
    ...
    for ipoint in range(Xnum.shape[0]):
        clust = np.argmin(
            num_costs[ipoint] + gamma * cat_dissim(centroids[1], Xcat[ipoint], X=Xcat, membship=membship)
        )
...
def k_prototypes_single(Xnum, Xcat, nnumattrs, ncatattrs, n_clusters, n_points,
    ...
    while True:
        ...
        #     centroids = kmodes.init_huang(Xcat, n_clusters, cat_dissim, random_state) # 至此为止是离散数据的初始中心
        ...
        numAndCat = np.zeros((len(Xnum),len(Xnum[0])+len(Xcat[0])),dtype=float)
        for i in range(0,len(Xnum)):
            for j in range(0,len(Xnum[0])):
                numAndCat[i][j] = Xnum[i][j]
        for i in range(0,len(Xcat)):
            for j in range(0,len(Xcat[0])):
                numAndCat[i][len(Xnum[0]) + j] = Xcat[i][j]

        radius = 10
        leastAdjacentPointNum = len(Xnum)/n_clusters
        disenty = {}
        minimumDis = {}

        for i in range(0,len(numAndCat)):
            disenty[i] = 0
            minimumDis[i] = sys.maxsize
            for j in range(0,len(numAndCat)):
                dis = np.linalg.norm(numAndCat[i]-numAndCat[j])
                if dis != 0:
                    minimumDis[i] = min(minimumDis[i],dis)
                if  dis < radius:
                    disenty[i] += 1
        qualifiedPoints = []
        for i in disenty.keys():
            if disenty[i] > leastAdjacentPointNum:
                qualifiedPoints.append(i)
        qualifiedMinDis = {}
        for i in qualifiedPoints:
            qualifiedMinDis[i] = minimumDis[i]

        qualifiedMinDis = sorted(qualifiedMinDis.items(),key=operator.itemgetter(1),reverse=True)

        centIdx = []
        for i in range(0,n_clusters):
            centIdx.append(qualifiedMinDis[i][0])

        numArr = []
        catArr = []
        for ci in centIdx:
            numArr.append(Xnum[ci])
            catArr.append(Xcat[ci])
        numArr = np.array(numArr)
        catArr = np.array(catArr)
        centroids = [numArr,catArr]
        # print(centroids)
        # ------------------------------------------------------------------------------
...
class KPrototypes(kmodes.KModes):
    ...
    num_dissim : func, default: 创新点2 陶卓距离算法 num_TZ_dissim
        Dissimilarity function used by the algorithm for numerical variables.
        Defaults to the Euclidian dissimilarity function.

    cat_dissim : func, default: 创新点2 陶卓距离算法 cat_TZ_dissim
    ...
    def __init__(self, n_clusters=8, max_iter=100, num_dissim=num_TZ_dissim,
